# -*- coding: utf-8 -*-
"""
Main script for running the 7-fold CV of the 1D CNN + DNN-based classifier
proposed in [1] for identification of the Brugada syndrome.
...
Created on Fri Jun 28 12:15:16 2024
...
# Load the dataset
X, Xm, Xp, y = ut.load_data(data_folder, N_L)
...
for train, test in kfold.split(X, y):
    ...
    # Select the model
    net = models.DNN_CNN(LR, N_L)

    # Early stopping
    early_stop = EarlyStopping(monitor='loss', patience=5, restore_best_weights=True)

    # Train the selected model
    history = net.fit([X[train], Xm[train], Xp[train]], y[train], batch_size=N_b, epochs=N_e,
    ...
    # Evaluate the model
    y_prob = net.predict([X[test], Xm[test], Xp[test]])
