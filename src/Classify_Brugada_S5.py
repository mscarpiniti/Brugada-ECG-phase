# -*- coding: utf-8 -*-
"""
Main script for running the 7-fold CV of the S5 strategy classifier proposed in
[1] for comparing the identification performance on the Brugada syndrome in [2].

Data are available at: https://datadryad.org/stash/dataset/doi:10.5061/dryad.s1rn8pkd9


[1] Scarpiniti, M.: Arrhythmia detection by data fusion of ECG scalograms and
phasograms. (submitted to) Expert Systems With Applications (2024).

[2] M. Scarpiniti and A. Uncini, "Exploiting phase information for the
...
Created on Fri Jun 28 13:49:58 2024
...
# Load the dataset
Xm, Xp, y = ut.load_data_S5(data_folder, N_L)
...
for train, test in kfold.split(Xm, y):
    ...
    # Select the model
    net = models.S5(LR, N_L)

    # Early stopping
    early_stop = EarlyStopping(monitor='loss', patience=5, restore_best_weights=True)

    # Train the selected model
    history = net.fit([Xm[train], Xp[train]], y[train], batch_size=N_b, epochs=N_e, shuffle=True, callbacks=[early_stop])
    ...
    # Evaluate the model
    y_prob = net.predict([Xm[test], Xp[test]])
